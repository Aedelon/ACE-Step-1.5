...
def generate_with_batch_management(
    ...
    if result is None:
        error_msg = t(
            "messages.batch_failed", error="No generation result was produced"
        )
        logger.warning(
            "[generate_with_batch_management] generate_with_progress yielded no results"
        )
    ...
    generated_codes_batch = (
        raw_codes_list if isinstance(raw_codes_list, list) else [""] * 8
    )
    ...
    extra_outputs_from_result = (
        result[46] if len(result) > 46 and result[46] is not None else {}
    )
    ...
    if auto_lrc and extra_outputs_from_result:
        batch_queue[current_batch_index]["lrcs"] = extra_outputs_from_result.get(
            "lrcs", [""] * 8
        )
        batch_queue[current_batch_index]["subtitles"] = extra_outputs_from_result.get(
            "subtitles", [None] * 8
        )
    ...
    logger.info(
        f"[generate_with_batch_management] Final yield: {len(ui_core_list)} core + 9 state"
    )
