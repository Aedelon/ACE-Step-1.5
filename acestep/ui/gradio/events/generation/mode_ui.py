...
def compute_mode_ui_updates(mode: str, llm_handler=None, previous_mode: str = "Custom"):
    ...
    show_generate_row = (
        True  # Always show Generate button (Smart Generate in Simple mode)
    )
    ...
    if is_extract or is_lego or is_cover or is_repaint:
        think_update = gr.update(
            interactive=False, value=False, visible=not (is_extract or is_lego)
        )
    ...
    show_results = True  # Always show results (Smart Generate in Simple mode)

    # Generate button label
    if is_extract:
        generate_btn_update = gr.update(
            interactive=generate_interactive, value=t("generation.extract_stem_btn")
        )
    elif is_lego:
        generate_btn_update = gr.update(
            interactive=generate_interactive, value=t("generation.add_stem_btn")
        )
    else:
        generate_btn_update = gr.update(
            interactive=generate_interactive, value=t("generation.generate_btn")
        )

    # --- Extract/Lego-mode outputs (indices 19-29) ---
    captions_update, lyrics_update, bpm_update, key_scale_update = (
        _compute_field_updates_for_mode(
            is_extract,
            is_lego,
            not_simple,
            leaving_extract_or_lego,
        )
    )
    time_signature_update, vocal_language_update, audio_duration_update = (
        _compute_meta_updates_for_mode(
            is_extract,
            is_lego,
            not_simple,
    ...
    auto_score_update, autogen_update, auto_lrc_update, analyze_btn_update = (
        _compute_automation_updates(
            is_extract,
            is_lego,
            not_simple,
    ...
    repainting_header_update, repainting_start_update, repainting_end_update = (
        _compute_repainting_labels(
            is_lego,
            is_repaint,
        )
    ...
    return (
        ...
        gr.update(
            info=mode_help_text, elem_classes=["has-info-container"]
        ),  # 12: generation_mode
