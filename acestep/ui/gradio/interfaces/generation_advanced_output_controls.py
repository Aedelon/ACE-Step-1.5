...
def _update_mp3_control_visibility(audio_format: str, service_mode: bool = False):
    ...
    return (
        ...
        gr.update(
            choices=_MP3_BITRATE_CHOICES, visible=visible, interactive=interactive
        ),
        gr.update(
            choices=_MP3_SAMPLE_RATE_CHOICES, visible=visible, interactive=interactive
        ),
    )
...
def build_output_controls(
    ...
    with gr.Accordion(
        t("generation.advanced_output_section"),
        open=False,
        ...
        with gr.Row():
            enable_normalization = gr.Checkbox(
                ...
                value=params.get("enable_normalization", True)
            ...
            normalization_db = gr.Slider(
                ...
                maximum=0.0,
                step=0.1,
                value=params.get("normalization_db", -1.0)
                if service_pre_initialized
                else -1.0,
        ...
        with gr.Row():
            fade_in_duration = gr.Slider(
                ...
                maximum=10.0,
                step=0.1,
                value=params.get("fade_in_duration", 0.0)
                if service_pre_initialized
                else 0.0,
            ...
            fade_out_duration = gr.Slider(
                ...
                maximum=10.0,
                step=0.1,
                value=params.get("fade_out_duration", 0.0)
                if service_pre_initialized
                else 0.0,
        ...
        with gr.Row():
            latent_shift = gr.Slider(
                ...
                maximum=0.2,
                step=0.01,
                value=params.get("latent_shift", 0.0)
                if service_pre_initialized
                else 0.0,
            ...
            latent_rescale = gr.Slider(
                ...
                maximum=1.5,
                step=0.01,
                value=params.get("latent_rescale", 1.0)
                if service_pre_initialized
                else 1.0,
